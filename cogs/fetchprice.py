...
class FetchPrice(commands.Cog):
    ...
    @commands.command(aliases=[
        'Prices', 'price', 'Price', 'PRICE', 'PRICES', 'quick', 'Quick',
        'QUICK'
    ])
    async def prices(self, ctx, *, item):
        ...
        try:
            ...
            em.add_field(name='Locations',
                         value=embedLocationString,
                         inline=True)
            em.add_field(name='Min Sell Price',
                         value=embedPriceString,
                         inline=True)
            em.add_field(name='Last Updated',
                         value=embedTimeString,
                         inline=True)
    ...
    def item_match(self, inputWord, option='difflib'):
        ...
        itemNames = [
            data[jDist[1]]['LocalizedNames']['EN-US'] for jDist in jDists[:4]
        ]
    ...
    def grabHistory(self, item, itemName):
        ...
        for i in range(1, 1 + numDays):
            dates[i - 1] = (today -
                            DT.timedelta(days=i)).strftime('%m-%d-%Y')
            ...
            try:
                ...
                for price in prices:
                    if price['location'] == 'Bridgewatch':
                        prices_minAll[0].extend(
                            price['data']['prices_min'])
                        timestampsAll[0].extend(
                            price['data']['timestamps'])
                    elif price['location'] == 'Caerleon':
                        prices_minAll[1].extend(
                            price['data']['prices_min'])
                        timestampsAll[1].extend(
                            price['data']['timestamps'])
                    elif price['location'] == 'Fort Sterling':
                        prices_minAll[2].extend(
                            price['data']['prices_min'])
                        timestampsAll[2].extend(
                            price['data']['timestamps'])
                    elif price['location'] == 'Lymhurst':
                        prices_minAll[3].extend(
                            price['data']['prices_min'])
                        timestampsAll[3].extend(
                            price['data']['timestamps'])
                    elif price['location'] == 'Martlock':
                        prices_minAll[4].extend(
                            price['data']['prices_min'])
                        timestampsAll[4].extend(
                            price['data']['timestamps'])
                    elif price['location'] == 'Thetford':
                        prices_minAll[5].extend(
                            price['data']['prices_min'])
                        timestampsAll[5].extend(
                            price['data']['timestamps'])
        ...
        for (i, timestamps) in enumerate(timestampsAll):
            try:
                ...
                x, y = [
                    list(x) for x in zip(
                        *sorted(zip(timestampsAll[i], prices_minAll[i]),
        ...
        plt.title(
            f'Historical Minimum Sell Order Prices for {itemName} ({item})'
        )
        plt.xlabel('Dates')
        plt.ylabel('Silvers')
        plt.legend()
